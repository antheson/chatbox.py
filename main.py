...
def load_data():
    ...
    # ── DISCOUNT SIMULATION ──────────────────────────────────────────────────
    # original_price is fully null in this dataset, so we simulate realistic
    # sale discounts using a fixed seed so values are stable across reruns.
    import numpy as np
    rng = np.random.default_rng(seed=42)
    on_sale_mask = rng.random(len(df)) < 0.35   # ~35% of items on sale
    disc_choices = rng.choice(
        [10, 15, 20, 25, 30, 40, 50],
        size=len(df),
        p=[0.20, 0.20, 0.25, 0.15, 0.10, 0.07, 0.03]
    )
    df['discount_pct'] = 0
    df.loc[on_sale_mask, 'discount_pct'] = disc_choices[on_sale_mask]
    df['original_price'] = df.apply(
        lambda r: round(r['price'] / (1 - r['discount_pct'] / 100), 2)
        if r['discount_pct'] > 0 and r['price'] > 0 else r['price'],
        axis=1
    )
    # ─────────────────────────────────────────────────────────────────────────
...
def correct_intent_typo(user_input):
    ...
    intent_keywords = {
        ...
        # ── NEW: discount typo corrections ──
        'discount':   ['disount', 'discont', 'discout', 'dicount', 'discound', 'discuont'],
        'sale':       ['slae', 'salee', 'sael', 'ssale'],
        'discounted': ['discountd', 'discunted', 'discounted'],
    }
...
def generate_explanation(row, filters):
    ...
    # ── NEW: show discount pill prominently when user searched for discounts ──
    ...
    # ── NEW: discount pill — show first when intent is discount ──
    if intent == 'discount' and discount_pct and int(discount_pct) > 0:
        pills.insert(0, f"🔥 {int(discount_pct)}% off")
    ...
    # Priority 3 — discount (only if notable, for non-discount searches)
    if intent != 'discount':
        try:
...
def display_product_detail(row):
    ...
    # ── NEW: show discount badge on detail page ──
    discount_pct = row.get('discount_pct', 0)
    ...
    with col_info:
        ...
        # ── NEW: show sale badge + original price if discounted ──
        if discount_pct and int(discount_pct) > 0 and orig_price and float(orig_price) > float(price) > 0:
            st.markdown(
                f"**💰 Price:** {price_str} &nbsp;"
                f"<span style='background:#e53935;color:white;border-radius:6px;"
                f"padding:2px 8px;font-size:12px;font-weight:700;'>🔥 {int(discount_pct)}% OFF</span>",
                unsafe_allow_html=True
            )
            st.markdown(f"<s style='color:#999;font-size:13px;'>Was: ${float(orig_price):.2f}</s>", unsafe_allow_html=True)
        else:
...
def display_products(df_result, label="Recommended Products", card_key_prefix="card", filters=None):
    ...
    for i, (idx, row) in enumerate(df_result.iterrows(), start=1):
        ...
        price        = row.get('price', 0)
        price_str    = f"${price:.2f}" if price and price > 0 else "N/A"
        # ── NEW: build discount badge string for card ──
        disc         = row.get('discount_pct', 0)
        orig_p       = row.get('original_price', 0)
        discount_badge = (
            f"<span style='background:#e53935;color:white;border-radius:4px;"
            f"padding:1px 6px;font-size:11px;font-weight:700;margin-left:6px;'>🔥 {int(disc)}% OFF</span>"
            f"<s style='color:#aaa;font-size:11px;margin-left:4px;'>${float(orig_p):.0f}</s>"
        ) if disc and int(disc) > 0 and orig_p and float(orig_p) > float(price) > 0 else ""
        ...
        with col_card:
            ...
            st.markdown(
                ...
                f'<span style="font-weight:700;font-size:16px;color:#2e7d32;">{price_str}{discount_badge}</span>'
                f'</div>'
...
def extract_filters(text):
    ...
    # ── NEW: discount filter detection ────────────────────────────────────────
    discount_terms = [
        'discount', 'discounted', 'sale', 'on sale', 'clearance',
        'deal', 'deals', 'offer', 'offers', 'promo', 'promotion',
        'reduced', 'markdown', 'marked down', 'price drop',
    ]
    if any(term in text_lower for term in discount_terms):
        filters['discount_only'] = True

    # "X% off" → set minimum discount percentage
    pct_match = re.search(r'(\d{1,2})\s*%\s*(?:off|discount|sale|cheaper|reduction)', text_lower)
    if pct_match:
        filters['min_discount'] = int(pct_match.group(1))
        filters['discount_only'] = True

    # "at least X% off" / "minimum X% off"
    at_least_match = re.search(r'(?:at least|minimum|min)\s+(\d{1,2})\s*%\s*off', text_lower)
    if at_least_match:
        filters['min_discount'] = int(at_least_match.group(1))
        filters['discount_only'] = True
    # ─────────────────────────────────────────────────────────────────────────
...
def search_by_product_name(user_input):
    ...
    filter_words = {
        ...
        # ── NEW: prevent "sale", "discount" etc. from hitting name search ──
        'sale', 'discount', 'discounted', 'deal', 'deals', 'offer', 'promo',
        'clearance', 'reduced', 'markdown',
    }
...
def get_response(user_input):
    ...
    if name_results is not None and not name_results.empty:
        filters = {'category': None, 'color': None, 'min_price': None,
                   ...
                   'discount_only': False, 'min_discount': None}
    ...
    if not filters['category'] and not filters['color'] and not filters['min_price'] and not filters['max_price'] and not filters.get('subcategory') and not filters.get('discount_only'):
    ...
    # ── NEW: apply discount filter ────────────────────────────────────────────
    if filters.get('discount_only'):
        min_disc = filters.get('min_discount') or 1
        result = result[result['discount_pct'] >= min_disc]
    # ─────────────────────────────────────────────────────────────────────────
    ...
    if result.empty:
        ...
        # ── NEW: include discount in empty-result conditions ──
        if filters.get('min_discount'):
            conditions.append(f"at least {filters['min_discount']}% off")
        elif filters.get('discount_only'):
            conditions.append("on sale")
    ...
    # ── NEW: discount intent → sort by discount_pct descending ──
    if filters.get('discount_only') and filters['intent'] not in ('cheap', 'expensive', 'best'):
        sorted_result = result[result['discount_pct'] > 0].sort_values('discount_pct', ascending=False)
        if filters.get('min_discount'):
            msg = f"Here are products with {filters['min_discount']}%+ off 🔥"
        else:
            msg = "Here are discounted products (biggest savings first) 🔥"
    elif filters['intent'] == 'expensive':
    ...
    return {
        "type": "dataframe",
        "message": msg,
        "data": page_result[[c for c in cols_needed if c in page_result.columns]],
        "filters": filters
    }
...
if not st.session_state.welcomed:
    welcome_html = (
        ...
        # ── NEW: discount tile in welcome card ──
        "<div style='background:rgba(255,255,255,0.1);border-radius:8px;padding:8px 12px;color:#ffffff;font-size:13px;'>🔥 <b>Discounts</b><br><span style='color:#c5cae9;'>discounted shoes · 20% off · sale clothing</span></div>"
        "</div>"
